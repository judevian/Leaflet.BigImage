...
const patternimg = {};
var arr = ["dots","zacken","zacken2Neg","zacken2","linediagonal","linehorizontal", "linevertical","square", "circle","dotNeg","linehorizontalNeg","lineverticalNeg", "squareNeg", "circleNeg", "zackenNeg"  ];
for(var i = 0;i < arr.length;i++) {
	patternimg[arr[i]] = new Image();
	patternimg[arr[i]].src = "pattern/" + arr[i] + ".png";
}
...
(function (factory, window) {
    ...
    L.Control.BigImage = L.Control.extend({
        ...
        _getPathLayer: function (layer, resolve) {
            ...
            latlngs.forEach((latLng) => {
                if(latLng.constructor === Array) {
                    /**
                     *  MulitLineStrings where not supported. The following code checks for MultiLineStrings and stores the parts of the lines in an array and handles the printing in the _drawPath function
                     */
                    var subParts = [];
                    latLng.forEach((latLngSub) => {
                        let pixelPoint = self._map.project(latLngSub);
                        pixelPoint = pixelPoint.subtract(new L.Point(self.bounds.min.x, self.bounds.min.y));
                        subParts.push(pixelPoint);
                        if (pixelPoint.x < self.canvas.width && pixelPoint.y < self.canvas.height) correct = 1;
                    });
                    if (correct) parts.push(subParts);
                }
                else {
        ...
        _drawPath: function (value) {
            /**
             * This Code has been modified to handle MultiLineString  
             */
            let self = this;
            let count = 0;
            ...
            value.parts.forEach((point) => {
                if(point.constructor === Array) {
                    ...
                    point.forEach((subPoint) => {
                        self.ctx[count++ ? 'lineTo' : 'moveTo'](subPoint.x, subPoint.y);
                    });
                    // close path
                    if (value.closed) self.ctx.closePath();
                    this._feelPath(options);
        ...
        _feelPath: function (options) {
            if (options.fill) {
				if(typeof(options.pattern) != "undefined" && temporaryLBPGenPictureType == "raster") {
					var pattern = this.ctx.createPattern(patternimg[options.pattern], "repeat");
            ...
            if (options.stroke && options.weight !== 0) {
				...
				if(typeof(options.pattern) != "undefined") this.ctx.strokeStyle = "#000000";
				else this.ctx.strokeStyle = options.color;
